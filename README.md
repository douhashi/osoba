...
- **ファイルシステムアクセス**: Claudeがプロジェクト内の全ファイルを読み書きできます
- **コマンド実行**: Claudeが任意のシェルコマンドを実行できます
- **ネットワークアクセス**: 外部APIやサービスへのアクセスが可能です
- **機密情報の取り扱い**: 設定ファイルや環境変数にアクセスできる可能性があります

### 推奨する安全な使用方法

1. **信頼できる環境での使用**
   - 本番環境ではなく、開発専用環境で使用
   - 機密情報を含まないプロジェクトでの使用を推奨

2. **事前の準備**
   - `.gitignore`で機密ファイルを適切に除外
   - 環境変数に機密情報を含めない
   - 定期的なバックアップの実施

3. **監視とレビュー**
   - Claudeが生成したコードの必須レビュー
   - 実行されるコマンドの事前確認
   - ログファイルの定期的な確認
...
claude:
  phases:
    plan:
      args: ["--read-only"]  # 計画フェーズは読み取り専用
    implement:
      args: ["--interactive"]  # 実装フェーズは対話的に実行
    review:
      args: ["--read-only"]   # レビューフェーズは読み取り専用
```
